"""双色球预测系统 - 极简主入口 (ml/main.py)
    ...
    python -m ml.main train-predict-batch --m lightgbm --cols all
    python -m ml.main train-predict --m lstm
    python -m ml.main train-predict --m cnn_reg
...
_MODEL_MAP = {
    ...
    "lightgbm": ("ml.models.lgb_model", "LightGBMModel"),
    "lstm": ("ml.models.lstm_model", "LSTMModel"),
    "cnn_reg": ("ml.models.cnn_model", "CNNMathModel"),
    "transformer": ("ml.models.transformer_model", "TransformerAllModel"),
...
def run_train(mt, df, retrain=True, col=None):
    ...
    if mt in ["rf", "lightgbm"]:
        ...
        X, y = df, None
    elif "lstm" in mt or mt == "transformer":
        ...
        args.update({"red_cols": RED_COLS, "blue_col": "Blue1"})
    ...
    m.train(X, (yc, yr) if mt == "cnn_reg" else y)
    m.save(sd)
    return m
...
def run_predict(mt, df, col=None):
    ...
    ws = m.config.get("window_size", 128 if ("lstm" in mt or mt == "transformer") else 33)
    
    if "lstm" in mt or mt == "transformer":
...
def main():
    ...
    tb = sp.add_parser("train-predict-batch"); tb.add_argument("-m", "--model", required=True, choices=["rf", "lightgbm"])
