...
| 2026-08-29 | **B 档: 奖金数据采集 + EV 量化(用户拍板 B: 建新表 + 抓数据试探)**。背景: 08-29 研究简报提出奖池 EV 阈值[1]/本土化[3], 但探针发现 draw_history 从无奖金字段, 简报"3494期奖池已存在"假设错误。落地: ①新建 ssq.draw_stats(dnum PK + sales BIGINT + poolmoney BIGINT + prizegrades JSONB, FK→draw_history), 隔离奖金数据不影响主流程; ②update_ssq.parse_cwl_latest 多解析 sales/poolmoney/prizegrades(同一 CWL 响应已含, 无需新爬虫), 主流程 upsert 到 draw_stats; ③research/backfill_draw_stats.py 回补 2013-01-01 起全量 → 写入 2056 期(1 期无 draw_history 对应跳过); ④analysis/ssq_ev.py 算每期单位 EV(事前期望 + 事后反推两口径)。核心结论(可证伪落点已验证): 事前 EV 全样本恒负(均值 -1.57元/注, 最优 -1.15, 0 期正 EV 窗口) → 印证理性停手量化证据, SSQ 封顶1000万+高销量使分奖碰撞压垮一等奖期望; 事后 EV 均值 +4.82 为幸存者偏差(假设预知低注数期), 不可用于决策, 仅结构参考。诚实边界: 事前模型用 λ=E[N·p] 期望近似分奖(乐观上界), 真实方差使 EV 更低, 故结论偏乐观仍恒负 → 极强证据。 |
