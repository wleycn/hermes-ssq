...
└── ml/                           # 源代码
    ...
    ├── data/1.csv                # 原始数据（3494 期开奖记录，CRLF）
...
| 表 | 用途 | 清理策略 |
|----|------|---------|
| `ssq.draw_history` | 1.csv 全量镜像归档（开奖历史），update_ssq 自动 upsert 写入，pg_schema 整表重建兜底 | **全量保留，永不清理** |
| `ssq.model_predictions` | 各模型红/蓝球预测概率 | **滚动 30 自然日清理**（见下） |

建表与导入：

```bash
.venv/bin/python pg_schema.py    # 幂等：建表 + 整表重建导入 draw_history(3494行) + 加 data_date 列；日常开奖由 update_ssq 自动 upsert，本命令为兜底/首次建库
```
...
> 开奖历史表 `draw_history` 硬编码排除在清理范围外。**update_ssq 已自动 upsert 写入 draw_history**（无需手动同步）；`pg_schema.py` 仅作整表重建兜底（函数幂等）。
...
> - **当前内容**：`0 4 * * 6,7 cd /home/hermes/workspace/python/SSQ/research && /home/hermes/workspace/python/SSQ/.venv/bin/python run_research.py >> logs/cron.log 2>&1`（2026-08-22 由每日降频为周6/7，Rocky 拍板；同日并入 SSQ 项目目录 research/）
...
> - **日志**：`research/logs/cron.log`（研究任务自身输出）
> - 权威 cron 布局见 `research/ARCHITECTURE.md` §1.2（本表为摘要）
...
| **功效 (power)** | 检验"真有结构时能发现它"的能力。样本少→功效低→即使有结构也可能测不出（FLAT 可能是"没测出来"而非"真没有"）。本系统 3494 期对多数检验够用，但对某些 NIST 子项仍偏少，结论带这个 caveat。 |
...
| **H1 决策（已废止）** | 旧架构铁律：draw_history 只读归档、PG 不同步开奖数据。已于 2026-08-26 由 Rocky 拍板废止——`update_ssq` 现在自动 upsert 写入 `draw_history`，`pg_schema.py` 仅作整表重建兜底。 |
